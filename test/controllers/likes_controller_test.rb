...
class LikesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create updates venue stats over turbo stream" do
    assert_difference("Like.count") do
      post venue_like_url(@venue), params: { from_list: true }, as: :turbo_stream
    end
  ...
  test "create from show page skips venue list refresh over turbo stream" do
    assert_difference("Like.count") do
    ...
    assert_equal Mime[:turbo_stream].to_s, response.media_type
    assert_includes response.body, %(target="like_toggle_venue_#{@venue.id}")
    assert_not_includes response.body, %(target="venues_quick_stats")
  end
  ...
  test "destroy updates venue stats over turbo stream" do
    ...
    assert_difference("Like.count", -1) do
      delete venue_like_url(like.venue), params: { from_list: true }, as: :turbo_stream
    end
  ...
  test "destroy from show page skips venue list refresh over turbo stream" do
    like = likes(:one)
    sign_in_as like.user
